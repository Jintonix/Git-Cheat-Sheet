...
\newcommand{\chapterunderlay}{
  \begin{tikzpicture}[remember picture,overlay]
    ...
    % Footer banner
    \node[anchor=south east] at ($(current page.south east)+ (1cm,2cm)$ )
      {\includegraphics[scale=1.3]{Images/footer_v3}};

    % Page number
    \node[anchor=north west] (pgnum) at ($(dhref) + (-7cm,3.2cm)$)
      {\huge\color{yellow!81!red} \textbf\thepage};

    % Vertical line
    \draw[line width=3pt, color=yellow!81!red] ($(dhref) + (-6.0cm,1.2cm)$) -- ++(1cm,3cm) coordinate (linend);
  \end{tikzpicture}
}
